...
def check(file):
    ...
    while f:
        ...
        if not file_contents:
            break
        i += 1
#        match = re.search(r'\w+\.addModel\(\"\w+\",\s*\w*.*(?=getParameter)', file_contents, re.IGNORECASE)
        match = re.search(r'(?P<output>(request\.setAttribute|addModel))\s*\((?P<params>.*?)(?P<keyparam>(getParameter|request\.))', file_contents, re.IGNORECASE)
        if match:
    ...
    try:
        if not listonly:
            ...
            if os.path.splitext(fname)[1].lower() in skipexts:
                ...
                check(fname)
                os.system("title scanning %s" % fname)
#                print 'checked:',fname
    except: pass
    vcount += 1 
