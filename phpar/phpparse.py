...
def p_expr_backticks(p):
    'expr : BACKTICKS expr BACKTICKS'
    p[0] = ast.Eval(p[2], lineno=p.lineno(1))
...
# Build the grammar
parser = yacc.yacc(tabmodule='parsetab',debug=0)
    ...
    while True:
        try:
            if s:
                ...
                if prompt == 'INITIAL': prompt = 'sparker'
                prompt += '> '
                s += raw_input(prompt)
        except EOFError:
            break
